...
$sql = 'select
  p.id, p.nome, if(md.coordtcc = p.id, "selected", "")  as sel,
  md.id matriz
from 
  professores p inner join vinculo v on p.id = v.id_prof
  left join matriz_disc md on md.id_curso  = p.id_colegiado 
...
$coordcoordTcc = Outros::qry($sql);
$selOptioncoordTcc = '';
foreach($coordcoordTcc as $e){ 
   $selOptioncoordTcc .= '<option value="'.$e->id.'"  '.$e->sel.' >'.$e->nome.'</option>';
}
