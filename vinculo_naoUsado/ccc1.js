...
async function getDBMD() {
  ...
  let a = id + ano;
  listaVinc = await fetch(`../api/vinculos.php?md=${a}`).then(resp => resp.json()).catch(error => false);
  if (listaVinc.length > 0){
    listaVinc.forEach(e => insereTable(e));
...
function preencheForm(id){
  let index = listaVinc.findIndex(e => e.id === id);
  let myObj = listaVinc[index];
